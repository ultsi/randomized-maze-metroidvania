...
var prerequisites: Callable
var buy_action: Callable

func _init(p_type: Type, p_shop_description: String, p_cost_range: Array[int], p_prerequisites: Callable, p_buy_action: Callable) -> void:
	type = p_type
...
func passes_prerequisites(state: GameState) -> bool:
	return prerequisites.call(state)
